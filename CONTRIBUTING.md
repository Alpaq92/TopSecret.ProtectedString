...
3. **Internal members consumed across assemblies keep TFM-invariant signatures.** `TopSecret.ProtectedBlob` and the TPM packages consume core internals (`AesGcmShim`, `MemoryLocker`, `DumpExclusion`, `LockedScratchPool`, `AllocatePinnedBytes`/`AllocatePinnedEncryptedState`, `ZeroBytes`/`ZeroOnly`, `ProtectorLifetime`, …) via `InternalsVisibleTo`. Put `#if` inside method bodies, never on the declarations of such members.
