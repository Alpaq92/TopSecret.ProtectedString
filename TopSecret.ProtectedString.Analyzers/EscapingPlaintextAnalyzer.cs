...
///   <c>plain.AsSpan().ToString()</c>, or <c>string.Concat(...,
///   plain, ...)</c> inside an <c>Access</c> callback.</item>
...
public sealed class EscapingPlaintextAnalyzer : DiagnosticAnalyzer
{
    ...
    public override ImmutableArray<DiagnosticDescriptor> SupportedDiagnostics =>
        ImmutableArray.Create(CopyToStringRule, CapturedArrayRule);
    ...
    private static void AnalyzeInvocation(SyntaxNodeAnalysisContext context)
    {
        ...
        // <param>.ToString() — flag.
        if (member.Name.Identifier.ValueText == "ToString" &&
            invocation.ArgumentList.Arguments.Count == 0 &&
            TryGetAccessParameterName(member.Expression, context, out var toStringTarget))
        {
            context.ReportDiagnostic(Diagnostic.Create(
                CopyToStringRule, invocation.GetLocation(), toStringTarget));
            return;
        }

        // string.Concat(... plain ...) and friends — surface the most common
        // accidental-stringification overloads. Anything that's a static
        // method on System.String or System.Text.StringBuilder which receives
        // the plaintext arg by value triggers TPS001.
