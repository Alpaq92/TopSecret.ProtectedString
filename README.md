...
- [Performance](#performance)
  ...
  - [Argon2id (credential KDF)](#argon2id-credential-kdf)
    - [Why `TopSecret.Cryptography.Argon2` instead of upstream](#why-topsecretcryptographyargon2-instead-of-upstream)
...
`TopSecret.ProtectedString` pulls in [`TopSecret.Cryptography.Argon2`](https://www.nuget.org/packages/TopSecret.Cryptography.Argon2) — a maintained fork of `Konscious.Security.Cryptography.Argon2` — as a transitive dependency for credential verification; see [why a fork](#why-topsecretcryptographyargon2-instead-of-upstream) below. On `net10.0-browser` it also brings in [`BouncyCastle.Cryptography`](https://www.nuget.org/packages/BouncyCastle.Cryptography), used only there for AES-GCM (see [`browser-wasm` support](#browser-wasm-support)). The optional satellite packages — [TPM-backed key wrapping](#key-at-rest-wrapping-opt-in-tiered) for Windows / Linux and the [`appsettings.json` binder](#configuration-binding-from-appsettingsjson) — are covered in their own sections.
...
// Works on browser-wasm too, at the default p=1 — see the browser-wasm
// section for why raising parallelism above 1 there still throws.
```
...
| `ComputeArgon2idHash(salt, iterations, memoryKb, parallelism, hashLengthBytes)` | Compute an Argon2id hash of the plaintext. OWASP-aligned defaults (`t=3`, `m=19 MiB`, `p=1`, 32-byte output). Works on the single-threaded browser runtime at the default `p=1`; throws `PlatformNotSupportedException` there if `parallelism` is raised above 1 (see [`browser-wasm` support](#browser-wasm-support)). |
| `VerifyArgon2idHash(expectedHash, salt, iterations, memoryKb, parallelism)` | Constant-time verification against a previously stored hash. Same browser behavior as `ComputeArgon2idHash`. |
...
#### Why `TopSecret.Cryptography.Argon2` instead of upstream

`ComputeArgon2idHash` depends on [`TopSecret.Cryptography.Argon2`](https://www.nuget.org/packages/TopSecret.Cryptography.Argon2), a maintained fork of [`Konscious.Security.Cryptography`](https://github.com/kmaragon/Konscious.Security.Cryptography), adopted 2026-07-04. The reason isn't cosmetic: upstream's synchronous `GetBytes(int)` is sync-over-async (`Task.Run(...).Result`) and its lane-processing loop unconditionally dispatches every lane — including the only lane that exists at `DegreeOfParallelism = 1`, this library's sole supported value — via `Task.Run`. On the single-threaded browser runtime, that deadlocks: `.Result` blocks the only thread while the queued work needs that same thread to proceed. Proven from upstream's own source, not assumed ([kmaragon/Konscious.Security.Cryptography#22](https://github.com/kmaragon/Konscious.Security.Cryptography/issues/22)).

The fork's fix (v2.0.0) runs the first lane inline on the calling thread and dispatches only the remaining `N-1` lanes via `Task.Run` — at `p = 1` that's zero dispatched tasks, so the KDF never touches the thread pool at all. v2.1.4 additionally wipes the KDF's own multi-MB lane working memory after use, closing a residual-copy gap that existed in both the fork and upstream (the same class of issue documented for BouncyCastle's AES-GCM path below). This wasn't taken on faith: before adopting it, the fork was built and its compiled `browser-wasm` output run live under a genuinely single-threaded host (`Environment.ProcessorCount == 1`), its RFC 9106 test vector was independently checked against the actual standard text, and the full swap was re-verified end-to-end — including a live browser run of this library's own demo — before merging.
...
The SDK floor is **.NET 10 SDK 10.0.300** (`global.json` enforces it with `rollForward: latestFeature`, so any 10.0.3xx-or-newer SDK satisfies it). The 10.0.300 floor exists because the analyzer references `Microsoft.CodeAnalysis.CSharp` 5.6.0, and that requires a matching host compiler — which only ships in SDK 10.0.300+. A contributor on an older SDK gets a clean "SDK 10.0.300 not found" error at restore time instead of a cryptic CS9057 mid-build. See [Maintainer notes — version pins worth knowing](#maintainer-notes--version-pins-worth-knowing) for the full rationale. Building the platform TFMs requires the corresponding workloads (`dotnet workload install android ios macos maccatalyst wasm-tools`).
...
- **Argon2id runs in the browser, at a permanent ceiling of `parallelism = 1`.** This library depends on [`TopSecret.Cryptography.Argon2`](#why-topsecretcryptographyargon2-instead-of-upstream), a fork adopted specifically because upstream Konscious's `GetBytes` is sync-over-async and cannot complete on a single OS thread. The fork's fix runs the KDF's one lane (at `p = 1`, the only value this library uses) inline on the calling thread instead of via the thread pool, so the hash never needs a second thread. `DegreeOfParallelism > 1` still throws `PlatformNotSupportedException` on a detected single-threaded host — real multi-lane parallelism genuinely needs multiple OS threads the browser runtime doesn't have today, so this ceiling is permanent, not a bug to close later (barring .NET's multithreaded-WASM work, which needs `SharedArrayBuffer` + cross-origin isolation most hosting setups don't have). The KDF's OWASP-default ~19 MiB memory-hard working set now genuinely allocates and executes inside WASM linear memory — real, measurable cost (multiple seconds on this project's own live demo) that previously never materialized because the call failed instantly; tune `memoryKb` down for browser deployments if that matters, understanding the security trade-off that implies. Verified by actually running it: this library's own demo executes `ComputeArgon2idHash` and `VerifyArgon2idHash` live in a real browser build before this text was written, not just compiled.
...
- [RFC 9106 — *Argon2 Memory-Hard Function for Password Hashing*](https://datatracker.ietf.org/doc/html/rfc9106) — the algorithm specification; its Section 5.3 Argon2id worked example is the external test vector this project's own dependency swap was checked against, byte-for-byte, before adoption.
- [kmaragon/Konscious.Security.Cryptography#22](https://github.com/kmaragon/Konscious.Security.Cryptography/issues/22) — the upstream deadlock this library's Argon2 dependency exists to fix; see [why `TopSecret.Cryptography.Argon2` instead of upstream](#why-topsecretcryptographyargon2-instead-of-upstream).
- [`Alpaq92/TopSecret.Cryptography`](https://github.com/Alpaq92/TopSecret.Cryptography) — the maintained fork this library depends on for Argon2id.
